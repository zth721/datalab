...
class FunctionFinder(c_ast.NodeVisitor):
    ...
    def visit_FuncDef(self, node):
        if node.decl.name == self.target_func_name:
            ...
            raise StopIteration
true_ans={"bitXor":{"rating":1,"score":0,"operation":["~","&"],"maxop":7},
          "samesign":{"rating":2,"score":0,"operation":[">>","<<","!","^","&&","if","else","&"],"maxop":12},
          "logtwo" :{"rating":4,"score":0,"operation":[">","<",">>","<<","|"],"maxop":25},
          "byteSwap":{"rating":4,"score":0,"operation":["!","~","&","^","|","+","<<",">>"],"maxop":17},
          "reverse":{"rating":3,"score":0,"operation":["<<","|","&","-","+",">>","for","while","!","~"],"maxop":30},
          "logicalShift":{"rating":3,"score":0,"operation":["!","~","&","^","|","+","<<",">>"],"maxop":20},
          "leftBitCount":{"rating":4,"score":0,"operation":["!","~","&","^","|","+",">>","<<"],"maxop":50},
          "float_i2f":{"rating":4,"score":0,"operation":["if","else","while","for","&","|","~","+","-","<<",">>","<",">","!"],"maxop":30},
          "floatScale2":{"rating":4,"score":0,"operation":["&","<<",">>","|","if",">","<",">=","<=","!","~","else","+"],"maxop":30},
          "float64_f2i":{"rating":3,"score":0,"operation":[">>","<<","|","&","~","!","+","-",">","<",">=","<=","if","else"],"maxop":60},
          "floatPower2":{"rating":4,"score":0,"operation":["<",">","<=",">=","<<",">>","+","-","&","|","~","!","if","else","&&"],"maxop":30}
}
def test_score(function,real_ans):
    ...
    if array_ok:
        ...
        for i in control_strcutures:
            if i not in true_ans[function_name]["operation"] and i.lower() not in true_ans[function_name]["operation"]:
                illegal.append(i)
                flag_operation=0
                break
        if flag_operation==0:
            ...
            if len(operators)>true_ans[function_name]["maxop"]:
                print(f"Exceed maxops,your ops are {len(operators)},we want {true_ans[function_name]['maxop']},name:{function_name},point:{0}")
                if real_ans[function_name]!=0:
                    real_ans["Total"]-=true_ans[function_name]["rating"]
            else:
                if real_ans[function_name]==0:
                    print(f"Please look at result.txt,you have some errors.name:{function_name},point:{0}")
                else:
                    print(f"Great!!! name:{function_name},point:{real_ans[function_name]}")
real_ans={}
